...
-- GetCompletedScales 'cfb6f0a4-1b5d-44d9-8ce8-d27e7d5b997b'
...
SELECT	ScaleId,
		Iteration,
		ScaleItemId,
		FirstVisitDate = (SELECT MIN(VisitDate) FROM #tmp WHERE ScaleId = t.ScaleId AND Iteration = t.Iteration),
		VisitDate = (SELECT MAX(VisitDate) FROM #tmp WHERE ScaleId = t.ScaleId AND Iteration = t.Iteration),
		Value = MIN(Value),
