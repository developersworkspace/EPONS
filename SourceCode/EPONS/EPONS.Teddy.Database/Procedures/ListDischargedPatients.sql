IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P'  AND SCHEMA_NAME([schema_id]) = 'EPONS' AND QUOTENAME(name) = '[ListDischargedPatients]')
DROP PROCEDURE [EPONS].[ListDischargedPatients] 
GO

CREATE PROCEDURE [EPONS].[ListDischargedPatients] 
@start INT,
@length INT,
...
(
	...
	(
		(
			[DeallocationTimestamp] IS NOT NULL
			AND 
			[UserId] = @userId
		)
		OR
...
EXEC [Audit].[CreateLogEntry] '[EPONS].[ListDischargedPatients]', 'Executed'
