IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P'  AND SCHEMA_NAME([schema_id]) = 'EPONS' AND QUOTENAME(name) = '[FindMeasurementToolsForVisitByPatientId]')
DROP PROCEDURE [EPONS].[FindMeasurementToolsForVisitByPatientId] 
GO

CREATE PROCEDURE [EPONS].[FindMeasurementToolsForVisitByPatientId] 
@patientId UNIQUEIDENTIFIER
AS
SELECT
[patientMeasurementTool].[MeasurementToolId] AS [MeasurementToolId],
...
ON 
[measurementTool].[MeasurementToolId] = [patientMeasurementTool].[MeasurementToolId]
AND
...
[subScoreItemValue].[ScoreValueId] [SubScoreValueId],
...
EXEC [Audit].[CreateLogEntry] '[EPONS].[FindMeasurementToolsForVisitByPatientId]', 'Executed'